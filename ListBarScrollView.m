...
@implementation ListBarScrollView
...
- (void)itemClick:(UIButton *)sender {
    if (self.btnSelect != sender) {
        ...
        [self.barDelegate selectListBarButtonAtIndex:[self findIndexOfListsWithTitle:sender.titleLabel.text] itemName:sender.titleLabel.text];
    ...
    [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            if (sender.frame.origin.x >= kScreenW - 150 && sender.frame.origin.x < self.contentSize.width - 200) {
                changePoint = CGPointMake(sender.frame.origin.x - 170, 0);
            } else if (sender.frame.origin.x >= self.contentSize.width - 200) {
                changePoint = CGPointMake(self.contentSize.width - 320, 0);
            } else {
