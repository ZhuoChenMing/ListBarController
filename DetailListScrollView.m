...
#import "ListItemButton.h"

@interface DetailListScrollView()

@property (nonatomic,strong) UIView *sectionHeaderView;

@property (nonatomic,strong) NSMutableArray *allItems;

@property (nonatomic,strong) ListItemButton *itemSelect;

@end

@implementation DetailListScrollView

- (void)setListAllAray:(NSMutableArray *)listAllAray {
    _listAllAray = listAllAray;
    self.showsVerticalScrollIndicator = NO;
    self.contentInset = UIEdgeInsetsMake(0, 0, 20, 0);
    self.backgroundColor = [UIColor whiteColor];
    
    NSArray *listTop = listAllAray[0];
    NSArray *listBottom = listAllAray[1];
...
#pragma 更多频道标签
    ...
    for (int i =0; i <count1; i++) {
        ListItemButton *item = [[ListItemButton alloc] initWithFrame:CGRectMake(padding+(padding+kItemW)*(i% itemPerLine), padding+(kItemH + padding)*(i/itemPerLine), kItemW, kItemH)];
        item.longPressBlock = ^(){
        ...
        item.location = top;
        [self.topViewArray addObject:item];
        item->locateView = self.topViewArray;
        item->topView = self.topViewArray;
        item->bottomView = self.bottomViewArray;
    ...
    for (int i=0; i<count2; i++) {
        ListItemButton *item = [[ListItemButton alloc] initWithFrame:CGRectMake(padding+(padding+kItemW)*(i%itemPerLine),CGRectGetMaxY(self.sectionHeaderView.frame)+padding+(kItemH+padding)*(i/itemPerLine), kItemW, kItemH)];
        item.operationBlock = ^(animateType type, NSString *itemName, int index){
            if (self.opertionFromItemBlock) {
                self.opertionFromItemBlock(type,itemName,index);
            }
        ...
        item.hitTextLabel = self.sectionHeaderView;
        [self.bottomViewArray addObject:item];
        item->locateView = self.bottomViewArray;
        item->topView = self.topViewArray;
        item->bottomView = self.bottomViewArray;
        [self addSubview:item];
        [self.allItems addObject:item];
...
- (void)itemRespondFromListBarClickWithItemName:(NSString *)itemName {
    for (int i = 0 ; i<self.allItems.count; i++) {
        ListItemButton *item = (ListItemButton *)self.allItems[i];
